...
function houseLens(house) {
  const lenses = {
    1: {
      sphere: "тело, образ, самопрезентация, первый импульс года и способ входить в события",
      events: ["смена образа, поведения или телесного режима", "ситуации, где нужно показать себя", "первые реакции людей на твою новую роль"],
      practice: "выбрать один видимый жест года: образ, границу, привычку тела или способ заявлять о себе",
      shadow: "строить личность из защиты, доказательства или чужой реакции",
    },
    2: {
      sphere: "деньги, тело, имущество, самоценность, право иметь и распоряжаться ресурсом",
      events: ["доходы и траты", "отношение к телу и комфорту", "проверки ценности своего труда"],
      practice: "посчитать ресурс и назвать, что нужно укрепить: деньги, тело, цену, запас или навык",
      shadow: "мерить себя только полезностью, доходом или внешней устойчивостью",
    },
    3: {
      sphere: "речь, обучение, мышление, документы, близкое окружение и короткие поездки",
      events: ["разговоры, переписки, обучение", "новости от близкого круга", "решения через слова, тексты и договоренности"],
      practice: "сформулировать мысль письменно и проверить, что именно нужно сказать, изучить или уточнить",
      shadow: "распыляться в словах, тревожных мыслях или чужом информационном поле",
    },
    4: {
      sphere: "дом, семья, род, недвижимость, внутренняя база и чувство защищенности",
      events: ["домашние решения", "семейные разговоры", "темы рода, жилья и внутренней опоры"],
      practice: "укрепить одну базовую опору: место, порядок, семейную границу или внутреннее право на дом",
      shadow: "застревать в старой семейной роли или защищать прошлое вместо живой базы",
    },
    5: {
      sphere: "творчество, романтика, дети, радость, игра и право быть живой",
      events: ["влюбленность, свидания, творческие проекты", "темы детей и удовольствия", "ситуации, где хочется сиять"],
      practice: "дать карте творческую форму: жест, текст, свидание, игру, маленький проект или удовольствие",
      shadow: "ждать разрешения на радость или превращать проявление в спектакль ради оценки",
    },
    6: {
      sphere: "режим, здоровье, обязанности, повседневная работа, сервис и забота о теле",
      events: ["нагрузка, расписание, здоровье", "рабочие процессы и повторяющиеся задачи", "отношения с обязанностями"],
      practice: "перевести карту в один повторяемый навык режима, тела или работы",
      shadow: "тащить обязанности автоматически и не слышать тело",
    },
    7: {
      sphere: "партнерство, брак, клиент, договор, открытый оппонент и тень, видимая через другого",
      events: ["важные разговоры один-на-один", "договоры, конфликты, притяжение", "проекции на партнера или клиента"],
      practice: "назвать, что карта показывает в другом, и какую часть этого нужно вернуть себе",
      shadow: "переносить всю карту на партнера и не видеть свой вклад в динамику",
    },
    8: {
      sphere: "интимность, кризис, чужие деньги, долги, страх, зависимость и глубокая трансформация",
      events: ["темы доверия, секса, долгов, налогов, чужих ресурсов", "кризисы и необратимые решения", "контакт с сильным страхом или желанием"],
      practice: "отделить реальный риск от фантазии и выбрать один зрелый способ обращаться с глубиной",
      shadow: "усиливать драму, контроль или зависимость вместо трансформации",
    },
    9: {
      sphere: "вера, смысл, дальние горизонты, обучение, путешествия, мировоззрение и наставники",
      events: ["курсы, поездки, учителя", "расширение картины мира", "выбор смысла, которому хочется служить"],
      practice: "сформулировать, чему карта учит как большой принцип года",
      shadow: "превращать смысл в догму, идеализацию или бегство от повседневных действий",
    },
    10: {
      sphere: "карьера, статус, цель, репутация, публичная видимость и взрослая ответственность",
      events: ["карьерные решения", "видимость, признание, статус", "разговоры с руководством или роль лидера"],
      practice: "перевести карту в цель года: что строить, чем управлять, где стать видимой",
      shadow: "доказывать ценность через статус или брать ответственность без живого смысла",
    },
    11: {
      sphere: "друзья, сообщества, проекты будущего, аудитория, союзники и коллективные идеи",
      events: ["группы, команды, аудитория", "новые проекты и планы", "выбор своих людей"],
      practice: "понять, с кем карта усиливается, а с кем теряет ресурс",
      shadow: "растворяться в группе, ожидать спасения от сообщества или терять личный голос",
    },
    12: {
      sphere: "скрытые процессы, бессознательное, одиночество, тайны, завершения и то, что не контролируется напрямую",
      events: ["сны, уединение, скрытые мотивы", "закулисные процессы", "ситуации отпускания и внутренней чистки"],
      practice: "дать карте тишину: записать сон, страх, повторяющийся образ или то, что пора отпустить",
      shadow: "теряться в тумане, тайне, спасательстве или избегании прямого действия",
    },
  };
  return lenses[Number(house.number)] || lenses[1];
}
...
function solarHouseFocus(card, house) {
  ...
  const lens = houseLens(house);
  const cardResource = firstText(card.resource, firstText(card.in_life, cardLine));
  const cardShadow = firstText(card.blind_spots || card.shadow, "искажение карты в этой теме");
  const cardPractice = firstText(card.practice_steps, "перевести карту в одно действие");
  return {
    formula: `${name} в ${house.name}: ${cardLine} проявляется через ${lens.sphere}.`,
    yearTask: `В этой связке дом задает сцену года: ${lens.sphere}. Карта ${name} показывает способ, которым эта сцена будет включаться: ${cardLine}. Поэтому трактовка не про “${name} вообще”, а про то, как именно эта карта влияет на ${lowerHouse}: через события, людей, тело, решения и повторяющиеся реакции в этой сфере.`,
    resource: [
      `Ресурс карты в этом доме: ${cardResource}.`,
      `Зрелое проявление дома: использовать тему “${lens.sphere}” как тренировочную площадку для карты ${name}.`,
      `Практический навык года: ${lens.practice}.`
    ],
    shadow: [
      `Тень карты здесь: ${cardShadow}.`,
      `Тень дома: ${lens.shadow}.`,
      `Слепая зона связки: видеть только событие в ${lowerHouse}, но не замечать, какую роль в нем играет карта ${name}.`
    ],
    practice: [
      `${lens.practice}.`,
      `${cardPractice}.`,
      `Записать один эпизод недели, где ${name} проявилась через ${lens.sphere}, и отметить ресурс/тень.`
    ],
    eventMarkers: [
      ...lens.events,
      ...((card.in_life || []).slice(0, 2)),
    ],
    questions: [
      `${houseQuestion} Через карту ${name}: ${cardLine}?`,
      `Где в теме “${lens.sphere}” уже виден ресурс карты: ${cardResource}?`,
      `Где эта же карта искажает дом через тень: ${cardShadow}?`,
      `Какой один практический шаг по дому сделать сейчас: ${lens.practice}?`
    ],
  };
}
